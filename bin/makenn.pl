...
$PI=3.14159265358979323846;
 $bvkA=25;
 $bvkalpha=1/9; # changed in mcphase 6.0 - because 3*3=9 ... i.e. easier to publish than sqrt(10) ...
...
# transform Qn=hkl to Qx Qy Qz(1/A)
if(@rkkynest){# rtoijk contains real lattice
# here we need reciprtocal lattice = 2pi * rtoijk^-1= 2pi * $invrtoijk
print "# Option rkkynest with scale =".$rkkynest[0]."meV and nesting vector(s):";
$hklnest="";
for($n=1;$n<$#rkkynest;$n+=3)
     {my $h=$rkkynest[$n]; 
      my $k=$rkkynest[$n+1]; 
      my $l=$rkkynest[$n+2]; 
     $hklnest=$hklnest."($h $k $l)";
      my $hkl=pdl($h,$k,$l);
      print $hkl;
      my $QxQyQz=2*$PI*($hkl x transpose($invrtoijk));
      $rkkynest[$n]=$QxQyQz->at(0,0);
      $rkkynest[$n+1]=$QxQyQz->at(1,0);
      $rkkynest[$n+2]=$QxQyQz->at(2,0);
#     print       $rkkynest[$n]." ".$rkkynest[$n+1]." ".$rkkynest[$n+2];
     }
 print "\n";
   ...
   {if ($djdeps1||$djdeps2||$djdeps3||$djdeps4||$djdeps5||$djdeps6) {die "Error makenn: option rkkynest does not provide direct dependence on strain epsilon\n";}
    ...
    for($n=1;$n<$#rkkynest;$n+=3)
     {my $Qx=$rkkynest[$n]; 
      my $Qy=$rkkynest[$n+1]; 
      my $Qz=$rkkynest[$n+2]; 
      my $Q=sqrt($Qx*$Qx+$Qy*$Qy+$Qz*$Qz);
      $jaa+=cos($Qx*$rx+$Qy*$ry+$Qz*$rz)/($Q*$Q*$Q*$r*$r*$r);
     }

   $jaa*=$rkkynest[0]; # scaling factor A
   $jbb =$jaa;$jcc =$jaa;
   $jab = 0;$jbc =0;$jac =0;$jba=0;$jcb=0;$jca=0; 
...
sub printneighbourlist {
  my ($Gmix,$h,$nofn,$gJ,$n,$an,$rn,$xn,$yn,$zn,$in,$jn,$kn,$Jaa,$Jbb,$Jcc,$Jab,$Jba,$Jac,$Jca,$Jbc,$Jcb)=@_;
     ...
     if (/^(#!|[^#])*diagonalexchange\s*=\s*/){
      ...
      if ($rkky>=1||($readtable>0&&$DM<1&&$Jp<1)||@rkkynest)
...
{push @atoms, ("# it follows output of RKKY interaction according to J(R)=A.sum_n cos(Qn.R)/(|Qn||R|)^3 with A=".$rkkynest[0]." meV and Qn=$hklnest generated by makenn\n");
}
elsif ($rkky==1)
