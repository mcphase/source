...
int read_pointcharge_parameters(graphic_parameters & gp,char ** sipffilenames,char * cffile,float * x, float *y, float*z,jjjpar & jjjps,Vector & abc)
...
while(pchere>3)
{
 if(jjjps.orientation==abc_yzx){printf("pointcharge %g |e| at xyz=%g %g %g mind: xyz=cab\n",par[1],par[2],par[3],par[4]);}
 // if(jjjps.module_type==0||jjjps.module_type==4) replaced by els 24.1.25 MR
 else{printf("pointcharge %g |e| at xyz=%g %g %g mind: xyz=abc\n",par[1],par[2],par[3],par[4]);}
