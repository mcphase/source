...
{fprintf(fout,"my $%s=new Math::Matrix (\n",name);
  complex<double> z;
  for (int i=1; i<=mat.nr(); i++) {
    fprintf(fout,"[");
    for (int j=1; j<=mat.nc(); j++) {
      z = mat[make_pair(i,j)];
      fprintf(fout,"%g %+g *i",real(z),imag(z));
      if(j<mat.nc()) fprintf(fout,","); 
    }
    fprintf(fout,"]");
    if(i<mat.nr()) fprintf(fout,",\n");
  }
  fprintf(fout,");\n");
}

int myReadComplexMatrix (FILE * file, zsMat<double> & M)
{int i1,j1;char instr[MAXNOFCHARINLINE];
 float *numbers;
  numbers = new float[M.nr()+2];
  numbers[0]=M.nr()+2;
...
{FILE * fin,*fout; 
 ...
 snprintf(perlfile,MAXNOFCHARINLINE,"results/_%s.pl",sipffilename);
...
// system call to perl
 snprintf(command,MAXNOFCHARINLINE,"perl results/_%s.pl\n",sipffilename);
  
if(system(command)){fprintf(stderr,"Error parsing sipffile through perl\n");return false; }

// read perl output and fill variables with values
 double dummy;
 snprintf(perlfile,MAXNOFCHARINLINE,"results/_%s.pl.out",sipffilename);
...
int myparse_execute(zsMat<double> **operators, char **operatornames, int *seq2, double *constval, char *statement, int ioc, int oe)
{
    ...
    zsMat<double> dummy, dummy2;
       ...
       if(*(t0+1)=='=') {                          // Operator-assignment [+=,-=,*=]
          ...
          if(seq2[2]<0) {
                snprintf(dummystr,MAXNOFCHARINLINE,"op(%i)[%s]",-seq2[2],operatornames[-seq2[2]]);
                dummy = (*operators[-seq2[2]]); i0=3; } else {              // Simple assignment
          switch(seq2[2]) {
             case 5:          snprintf(dummystr,MAXNOFCHARINLINE," %f * op(%i)[%s] ",constval[ioc],-seq2[4],operatornames[-seq2[4]]); is5=1;
             case 7: if(!is5){snprintf(dummystr,MAXNOFCHARINLINE," op(%i)[%s] * %f ",-seq2[4],operatornames[-seq2[4]],constval[ioc]);}is5=0;
                dummy =    constval[ioc++]     * (*operators[-seq2[4]]);    // Scalar*Matrix or Matrix*Scalar
                break;
             case 6: snprintf(dummystr,MAXNOFCHARINLINE," op(%i)[%s] * op(%i)[%s] ",-seq2[3],operatornames[-seq2[3]],-seq2[4],operatornames[-seq2[4]]);
          ...
          if(seq2[1]<0) {     snprintf(dummystr,MAXNOFCHARINLINE," op(%i)[%s] ",-seq2[1],operatornames[-seq2[1]]);
                (*operators[seq2[0]]) = (*operators[-seq2[1]]); i0=2; } else {              // Simple assignment
          switch(seq2[1]) {
             case 5:          snprintf(dummystr,MAXNOFCHARINLINE," %f * op(%i)[%s] ",constval[ioc],-seq2[3],operatornames[-seq2[3]]); is5=1;
             case 7: if(!is5){snprintf(dummystr,MAXNOFCHARINLINE," op(%i)[%s] * %f ",-seq2[3],operatornames[-seq2[3]],constval[ioc]);}is5=0;
                (*operators[seq2[0]]) =    constval[ioc++]     * (*operators[-seq2[3]]);    // Scalar*Matrix or Matrix*Scalar
                break;
             case 6: snprintf(dummystr,MAXNOFCHARINLINE," op(%i)[%s] * op(%i)[%s] ",-seq2[2],operatornames[-seq2[2]],-seq2[3],operatornames[-seq2[3]]);
       ...
       while(seq2[i0]!=0) {
          switch(seq2[i0]) {
             case 3:
             case 4:          snprintf(dummystr,MAXNOFCHARINLINE," op(%i)[%s] ",-seq2[i0+1],operatornames[-seq2[i0+1]]);
                dummy = (*operators[-seq2[i0+1]]); 
                pm = seq2[i0];   i0 += 2;
                break;
             case 5:          snprintf(dummystr,MAXNOFCHARINLINE," %f * op(%i)[%s] ",constval[ioc],-seq2[i0+2],operatornames[-seq2[i0+2]]); is5=1;
             case 7: if(!is5){snprintf(dummystr,MAXNOFCHARINLINE," op(%i)[%s] * %f ",-seq2[i0+2],operatornames[-seq2[i0+2]],constval[ioc]);}is5=0;
                dummy =      constval[ioc++]      * (*operators[-seq2[i0+2]]); // Scalar*Matrix or Matrix*Scalar
                pm = seq2[i0+3]; i0 += 4;
                break;
             case 6: snprintf(dummystr,MAXNOFCHARINLINE," op(%i)[%s] * op(%i)[%s] ",-seq2[i0+1],operatornames[-seq2[i0+1]],-seq2[i0+2],operatornames[-seq2[i0+2]]);
