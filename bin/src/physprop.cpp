   ...
   header[0]='\0';outstr[0]='\0';
    ...
    {ptr=NULL;
          if(i<15){switch(i) {       case 8: ptr=&fe;
                                     ...
                                     case 14: ptr=&mp;snprintf(hs,40,"m||(projection_along_Hext)[mb/ion]");break;
                                     default: ;
                              }
                   } 
           else   { // i>=15
...
{ FILE *fout;
  ...
  printf("saving properties for ");ini.print_usrdefcols(stdout,x,y,T,Hint,inputpars.cs.abc,M,P,true);
  if(ortho==0){printf(" Hi=%g Hj=%g Hk=%g ",Hint(1),Hint(2),Hint(3));}
...
//-----------------------------------mcphas.fum ----------------------------------------------------  
  ...
  fclose(fout);
   ...
   fout = fopen_errchk (outfilename,"a");ini.print_usrdefcols(fout,x,y,T,Hint,inputpars.cs.abc,M,P,false);
         fprintf(fout,"%s\n",outstr); fclose(fout);
...
//--------------------------------------mcphas.xyt---------------------------------------------------  
  ...
  fout = fopen_errchk (outfilename,"a");
   ...
   ini.print_usrdefcols(fout,x,y,T,Hint,inputpars.cs.abc,M,P,false);
    fprintf(fout,"%s\n",outstr);fclose(fout);
...
{ FILE *fin;int n;float nnerr[200];nnerr[0]=199;for(int i=1;i<=199;++i)nnerr[i]=0;
 ...
 if(verbose==1){ printf("reading properties for ");ini.print_usrdefcols(stdout,x,y,T,Hint,inputpars.cs.abc,M,P,true);}

//-----------------------------------------mcphas.fum------------------------------------------------  
// here read free energy etc if possible ... otherwise return 1
// check x y T Hint[1] Hint[2] Hint[3] agrees and fe nonzero ?
// then read fe, u, 
 ...
 while (feof(fin)==0&&0==(n=inputline(fin,nn))){}  // if yes -> input them
   ...
   if(ini.demag!=0){
    fumcols(nn,nnerr,n,false,str,outstr,ini,ortho,inputpars,verbose); // load parameters 
                   // Hint is in the case of demag=1 the 
    M=mu0M();      // field which is input (=external field) to ::read
    P=Pdiveps0();  // corrected by the demagnetising field
                                             // so that comparison with precalculated
                                             // internal fields is possible
                   }
    found=ini.checkTH(nn,T,Hint,inputpars.cs.abc,M,P);
         }
   fclose(fin);
   fumcols(nn,nnerr,n,false,str,outstr,ini,ortho,inputpars,verbose); // load parameters 
...
//fprintf (fout, "%4.4g %4.4g  %4.4g %4.4g %4.4g %4.4g %4.4g       %8.8g            %8.8g       %4.4g    %4.4g %4.4g %4.4g    %4.4g",
 //           myround(x),myround(y),myround(T),myround(Norm(Hijk)),myround(Hint[1]),
//myround(Hint[2]),myround(Hint[3]),myround(fe),myround(u),myround(Norm(m)),
...
//then read j ... not necessary
   //fprintf (fin, "%4.4g %4.4g %4.4g %4.4g %4.4g  %4.4g %4.4g       %ip           %ip      ",
     //       myround(x),myround(y),myround(T),myround(Norm(Hijk)),myround(Hint[1]),myround(Hint[2]),myround(Hint[3]),j,sps.wasstable);
...
//then read jj[] 
 ...
 // fprintf (fin, "%4.4g %4.4g   %4.4g %4.4g   %4.4g %4.4g %4.4g     ",myround(x),myround(y),myround(T),myround(Norm(Hijk)),myround(Hint[1]),myround(Hint[2]),myround(Hint[3]));
  //      for(j2=1;j2<=nofcomponents()*nofcomponents();++j2)               
...
//-----------------------------------------------------------------------------------------  
 errno = 0;
  ...
  //neutrons
   ...
   fin = fopen_errchk (infilename,"r");
   if (verbose==1)printf(" .... reading %s\n",infilename);
   // check x y T Hint[1] Hint[2] Hint[3] agrees and
  //then read hkli[1-4]
...
found=0;while(found==0){ i=0;
 while (feof(fin)==0&&0==i)i=inputline(fin,nn);  // if yes -> input them
   if (feof(fin)!=0) {fclose(fin);return 1;}
   found=ini.checkTH(nn,T,Hint,inputpars.cs.abc,M,P);nofhkls=(i-7)/4;
                       } 
   fclose(fin);
...
//then read sps  ... probably sps.read ...??
  ...
  // fprintf (fin, " %4.4g %4.4g %4.4g %4.4g %4.4g  %4.4g %4.4g %i %i %i ",
   //         myround(x),myround(y),myround(T),myround(Norm(Hijk)),myround(Hint[1]),myround(Hint[2]),myround(Hint[3]),sps.n()*sps.nofatoms,sps.nofatoms,sps.nofcomponents);
...
//fprintf (fin, " %4.4g %4.4g %4.4g %4.4g %4.4g  %4.4g %4.4g %i %i %i ",
 //           myround(x),myround(y),myround(T),myround(Norm(Hijk)),myround(Hint[1]),
  //         myround(Hint[2]),myround(Hint[3]),mf.n()*mf.nofatoms,mf.nofatoms,mf.nofcomponents);
