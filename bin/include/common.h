...
|						 			        |
| Last change: Apr 2, 2000							|
|                                                                               |
| Matpack Library Release 1.5.0                                                 |
| Copyright (C) 1991-2000 by Berndt M. Gammel. All rights reserved.             |
|                                                                               |
...
"Matpack C++ Numerics and Graphics Library, Version 1.6.1"

#define MP_COPYRIGHT_STRING \
"Copyright (c) 1991-2000 Berndt M. Gammel, All rights reserved"

#define MP_ADDRESS_STRING \
"Address: gammel@matpack.de"
...
//"Registered: Imperial College, London"

//----------------------------------------------------------------------------//
// include file with compatibility hacks for various architectures
//----------------------------------------------------------------------------//
...
//----------------------------------------------------------------------------//
// additional mathematical and physical constants (32 digits)
//----------------------------------------------------------------------------//
...
char*   BaseConvert     (long,long,char*);
int     yes  	 	(char*);
...
// --------
//
// The class 'Mat' isolates common definitions and prototypes, such as
// error types, print flags, etc..
//
// DESCRIPTION:
// ------------
//
// Matrix and vector print format flags (enum PrintFlags)
//
//	Mat::Text		readable text format with dimension information
//	Mat::TextData		same, but without dimension information (data only)
//	Mat::Binary		unreadable binary format with dimension information
//	Mat::BinaryData		same, but without dimension information (data only)
//
...
inline int	Nint (double d) { return (d>0) ? (int)(d+0.5) : -(int)(-d+0.5); }
...
inline double	Angle	(double x,double y) {return (x==0&&y==0)?0:atan2(y,x); }
inline float	Angle	(float x,float y) {return (x==0&&y==0)?0:atan2(y,x); }

// convert radians to degree and vice versa 

inline float	RadToDeg (int rad){return rad*180/M_PI;}
inline float	RadToDeg (long rad){return rad*180/M_PI;}
inline float	RadToDeg (float rad){return rad*180/M_PI;}
inline double	RadToDeg (double rad){return rad*180/M_PI;}

inline float	DegToRad (int deg){return deg*M_PI/180;}
inline float	DegToRad (long deg){return deg*M_PI/180;}
