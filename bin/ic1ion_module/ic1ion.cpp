      ...
      for(int ii=0; ii<(int)confLS.states.size(); ii++)
      {
         ...
         for(int mL2=-L2; mL2<=L2; mL2+=2)
         {
            for(int mS2=-S2; mS2<=S2; mS2+=2)
            {
               ...
               if(mL2%2==0) snprintf(Jlabel,12,",mL=%i",mL2/2); else snprintf(Jlabel,12,",mL=%i/2",mL2); conf.states[ns].id.append(Jlabel);
               if(mS2%2==0) snprintf(Jlabel,12,",mS=%i",mS2/2); else snprintf(Jlabel,12,",mS=%i/2",mS2); conf.states[ns].id.append(Jlabel);
//std::cout << "State: " << ns+1 << "\t";
...
void ic_printheader(const char *outfile, icpars &pars)
{
   ...
   std::string wy,stev; wy.assign("L");stev.assign("B");
