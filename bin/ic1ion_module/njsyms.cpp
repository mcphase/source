...
//                                        |J24-j3| <= g <= J24+j3
   ...
   min_g = G[0]; for(g=1; g<3; g++) if(G[g]>min_g) min_g = G[g]; // mind we should look for the maximum value of G(0,1,2) to find the minimum range boundary of g
   max_g = G[3]; for(g=4; g<6; g++) if(G[g]<max_g) max_g = G[g]; // mind we should look for the minimum value of G(3,4,5) to find the maximum range boundary of g
