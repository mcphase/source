...
\begin{table}[htb] 
\begin{center}  
\caption {Single ion\index{singleion} modules and standard operator sequence $\hat I_a,\hat I_b,\hat I_c ...$.}   
\label{simodtable}   
\begin{tabular} 
{l|l} 
...
\hline
{\prg bin/cf1ion\_module/so1ion.so} & $\hat J_a,\hat J_b,\hat J_c,\hat O_{20},\hat O_{22},\hat O_{40},\hat O_{42},\hat O_{44},\hat O_{60},\hat O_{62},\hat O_{64},\hat O_{66}$\\
 \end{tabular}
\end{center}   
\end{table}
...
In an alternative approach, changing the operator sequence may also be done internally
yet with less flexibility in parsing operators (addition, subtraction, multiplication).
The syntax of the same code as above will be
...
\hline
\hline
void Icalc              & {\prg mcphas,singleion}         & &calculates $\langle \hat \mbf I \rangle$\\
void IMcalc              &         & {\prg singleion}&the same with vectorized Temperature\\
{\tiny Icalc\_parameter\_storage\_matrix\_init}  && {\prg mcphas}       &parameter storage for Icalc,mcalc etc\\
   {\tiny (void)}                     && {\prg mcdiff}         &\\
						...
						&& {\prg densplt}        &\\
\hline
int du1calc				& {\prg mcdisp} &{\prg singleion}&calculates $\langle -|\hat \mbf I|+\rangle\sqrt{(p_--p_+)}$\\
...
\hline
\hline
void mcalc      	& & {\prg mcdiff,singleion} & magnetic moment $\langle\hat \mbf M\rangle$ [$\mu_B$]\\
	                & & {\prg mcphase} &\\
void mMcalc      	& & {\prg singleion} & the same with vectorized T\\
int dm1		& & {\prg mcdisp} & $\langle-| \hat\mbf M|+\rangle\sqrt{(p_--p_+)}$[$\mu_B$]\\
\hline
void Lcalc      	& & {\prg mcdiff,singleion} & orbital ang. momentum $\langle \hat \mbf L\rangle$[$\hbar$]\\
	                & & {\prg mcphase} &\\
void LMcalc      	& & {\prg singleion} & the same with vectorized T\\
int dL1		& & {\prg mcdisp} & $\langle-| \hat\mbf L|+\rangle\sqrt{(p_--p_+)}$[$\hbar$]\\
\hline
void Scalc      	& & {\prg mcdiff,singleion} & spin $\langle \hat\mbf S\rangle$[$\hbar$]\\
	                & & {\prg mcphase} &\\
void SMcalc      	& & {\prg singleion} & the same with vectorized T\\
int dS1		& & {\prg mcdisp} &  $\langle-|\hat \mbf S|+\rangle\sqrt{(p_--p_+)}$[$\hbar$]\\
\hline
void mqcalc      	& & {\prg mcdiff} & FT of magnetic moment $\langle\hat \mbf M(\mbf Q)\rangle$[$\mu_B$]\\
int dmq1		& & {\prg mcdisp} &  $\langle-|\hat \mbf M(\mbf Q)|+\rangle\sqrt{(p_--p_+)}$[$\mu_B$]\\
\hline
int drixs1 & & {\prg mcdisp} & RIXS transition operator $\hat \mbf R$,eq. (\ref{rixsop})\\
\hline
void pcalc      	& & {\prg mcphas} & phononic displacement $\langle \hat \mbf P\rangle$[$\AA$]\\
nt dP1 & & {\prg mcdisp} & phonon displacement $\langle-|\hat \mbf P|+\rangle\sqrt{(p_--p_+)}$[$\AA$]\\
\hline
void pelcalc      	& & {\prg mcphas} & electric dipole moment $\langle \hat \mbf p_{\rm el}\rangle$[$|e|$pm]\\
nt dpel1 & & {\prg mcdisp} & el. dipole moment $\langle-|\hat \mbf p_{\rm el}|+\rangle\sqrt{(p_--p_+)}$[$|e|$pm]\\
\hline
...
%*************************************************************************************
\subsubsection{External module function {\prg IMcalc} - used by 
{\prg singleion\index{singleion}} }

This function is optional and intended to accellerate {\prg singleion} by vectorizing the temperature and
thus save computation resources when using the option {\prg -Tsteps}. It has the same parameters as 
{\prg Icalc} with the following modifications:

\begin{enumerate}
\item  {\prg double $\ast$ T} is replaced by {\prg Vector \& T} containing the temperatures for which 
the calculation is to be done.
\item {\prg Vector \& I } is replaced by {\prg Matrix \& I} which should be filled with column
vectors containing the vector $\mbf I$. Each column corresponds to a temperature given as input{\prg Vector \& T}.
\item {\prg double $\ast$ U} is replaced by {\prg Vector \& U} which should be filled with
 the energies corresponding
to the temperatures in {\prg Vector \& T}
\item {\prg double $\ast$ lnZ} is replaced by {\prg Vector \& lnZ} which shoul be filled with the partition sums corresponding
to the temperatures in {\prg Vector \& T}
...
the magnetic moment vector $\mbf m$, which is returned, has always exactly 3 components.
%*************************************************************************************
\subsubsection{External module function {\prg mMcalc} - used by 
{\prg singleion\index{singleion}} }

This function is optional and intended to accellerate {\prg singleion} by vectorizing the temperature and
thus save computation resources when using the option {\prg -Tsteps}. It has the same parameters as 
{\prg mcalc} with the following modifications:

\begin{enumerate}
\item  {\prg double $\ast$ T} is replaced by {\prg Vector \& T} containing the temperatures for which 
the calculation is to be done.
\item {\prg Vector \& m } is replaced by {\prg Matrix \& m} which should be filled with column
vectors containing the  magnetic moments $\mbf m$. Each column corresponds to a temperature given as input{\prg Vector \& T}.
 \end{enumerate}
...
%*************************************************************************************
\subsubsection{External module function {\prg LMcalc} - used by 
{\prg singleion\index{singleion}} }

This function is optional and intended to accellerate {\prg singleion} by vectorizing the temperature and
thus save computation resources when using the option {\prg -Tsteps}. It has the same parameters as 
{\prg Lcalc} with the following modifications:

\begin{enumerate}
\item  {\prg double $\ast$ T} is replaced by {\prg Vector \& T} containing the temperatures for which 
the calculation is to be done.
\item {\prg Vector \& m } is replaced by {\prg Matrix \& L} which should be filled with column
vectors containing the  angular momentum $\mbf L$. Each column corresponds to a temperature given as input{\prg Vector \& T}.
 \end{enumerate}
...
%*************************************************************************************
\subsubsection{External module function {\prg SMcalc} - used by 
{\prg singleion\index{singleion}} }

This function is optional and intended to accellerate {\prg singleion} by vectorizing the temperature and
thus save computation resources when using the option {\prg -Tsteps}. It has the same parameters as 
{\prg Scalc} with the following modifications:

\begin{enumerate}
\item  {\prg double $\ast$ T} is replaced by {\prg Vector \& T} containing the temperatures for which 
the calculation is to be done.
\item {\prg Vector \& m } is replaced by {\prg Matrix \& L} which should be filled with column
vectors containing the  spin moment $\mbf S$. Each column corresponds to a temperature given as input{\prg Vector \& T}.
 \end{enumerate}
...
\subsubsection{External module function {\prg pcalc} - used by {\prg mcdiff\index{mcdiff}}
and {\prg mcphas}\index{mcphas}  }
...
\AA \ under the action of a Force {\prg Fxc} in units of meV, and external field
(electric field as components 4,5,6 of Hext in units of kV/mm).
...
\subsubsection{External module function {\prg dP1} - used by {\prg mcdisp\index{mcdisp}}}
...
\subsubsection{External module function {\prg pelcalc} - used by {\prg mcdiff\index{mcdiff}}
and {\prg mcphas}\index{mcphas}  }

In order to calculate the electric dipole moment  $\langle \hat \mbf p_{el} \rangle$
 the following function has to be present in the module file {\prg *.so}:
...
It returns the electric dipole moment vector {\prg pel}  in units of
$|e|$pm  under the action of a Force {\prg Fxc} in units of meV, and external field
(electric field as components 4,5,6 of Hext in units of kV/mm).
For the rest it works exactly as {\prg mcalc}, therefore we do not give more details here.

%*************************************************************************************

\subsubsection{External module function {\prg dpel1} - used by {\prg mcdisp\index{mcdisp}}}

In order to calculate neutron intensities for RAMAN scattering
the external single ion module has to provide the transition matrix elements of
 the electric dipole moment  $p_{{\rm el} \alpha}$ in units of $|e|$pm. The 

The format to be used is:
{\footnotesize
\begin{verbatim}
extern "C" int dpel1(int & tn,double & T,Vector & Hxc,Vector & Hext,double * g_J,Vector & MODPAR,
char ** sipffilename,ComplexVector & p1,float & maxE, ComplexMatrix & est)
\end{verbatim}
}
The requirements are equivalent to the {\prg dP1} function, therefore all details are not repeated here.
