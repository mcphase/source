\section{Including Phonons, Electrical Polarisation and Crystal-Field Phonon interactions}\label{phonons}

\EE I think you are now ready for the really interesting subjects - I mean phonons and their
interactions.

\PP \ee, you know, in the past 40 years there always was money for electronic studies and practically
no money for phonons and lattice dynamics. So why should we suddenly go back and study
phonons. You can predict phonon dynamics with DFT methods nowadays - even in exotic matter.
Are you serious about studying phonons ?

\EE I know, the mainstream of solid state research is based on the Born-Oppenheimer adiabatic
approximation and thus phonons are considered of minor importance. However,
BCS theory on superconductivity is a good example on how phonon may enter suddenly into
the theory with unexpected effects. Similar, in magnetism we usually neglect phonons. 
However, there are some examples which demonstrate, that for example for the
interpretation of crystal field spectra it may be necessary to take into
account compound modes having both phononic and magnetic character and correspondingly
can be observed in magnetic and nuclear scattering of neutrons.

\OO According to my simple view of this subject, aren't we talking about the origin
of magnetostriction ?

\EE Yes indeed, I did not think  this way. You are right, magnetostriction would
be zero if there was not some couling between the electronic and lattice degrees of 
freedom.

\PP So how can phonons be handled ?

\EE Let me first show you, how lattice dynamics may be considered in the framework of
...
to be done in {\prg mcdisp.j}. Rephrasing lattice dynamics in this way allows then
...
\begin{eqnarray}
\m{\chi}^0&=&\sum_{\nu\mu}\frac{\langle \nu|\hat \mbf u-\hat \mbf u_0|\mu\rangle\langle \mu |\hat \mbf u^T-\hat \mbf u_0^T|\nu \rangle}{\Delta_1 -\hbar \omega}
(p_{\nu} -p_{\mu}) \\
...
\subsection{Electric Fields and Polarisation}

In equation (\ref{phonsiham}) the force $\mbf F$ may also come from an
electric field $\mbf E_{\rm el}$, i.e.

\begin{equation}
\mbf F = q \mbf E_{\rm el} a_0
\end{equation}

The Bohr radius $a_0$ is included in order to yield $\mbf F$ in units of meV.
%Note that similar to the external magnetic field, also the electric field $\mbf E_{\rm el}$
% refers to the electric field in the sample, which is 
%- Eel is the internal electric field at the position in the sample, which is related to the
%  eletric field in the sample E. Note that E varies discontinously at the sample surface ( eps0 divE=- div Pel if
%there is no free charge)
% eps0 Eel = eps0 E + 1/3 Pel    equ. 4.73 p 119 in Jackson transcribed to SI units (Pel <-> 4pi Pel^esu) 
%(McPhase   Xel = N gamma_mol = Pel/(eps0 Eel), i.e. static electric susceptibility is Xe= Pel/(eps0 E)= Xel /(1- Xel/3)  )
%(in SI: dielectric constant is eps=1+Xe)
% Xelalphabeta= N pelalpha /eps0 Eelbeta  = N  sum_jk <j|pelalpha|k> <k|pelbeta|j>/(E-Ejk)  
% Xelpoly= 1/3 trace(Xel)
%// *** calculate electrical polarisation (static, from ionic charges and positions, electret matter)               

The resulting electric polarisation $\mbf P_{\rm el}$ can be calculated by {\prg mcphas}
and is stored in output file {\prg results/mcphas.fum}. This electric polarisation refers to
the orientational polarisation generated by moving ions from their equilibrium positions. It does
not include the molecular electric dipole moment induced by the electric field.
%- McPhase does not consider the induced molecular electric dipole moment (see Jackson p. 120)
%  it rather calculates an electric dipole moment induced by the displacement of ions !

\begin{equation}
\mbf P_{\rm el} = q_i  \langle \mbf \Delta r_i \rangle 
\end{equation}

% Problem : surface effect !! how to calculate polarisation !????
% try - use wigner seitz cell 
Note that in addtion there may be a static polarisation $\mbf P_{\rm el0}$(i.e. electret matter), which is calculated using
the Wigner Seitz unit cell of the primitive lattice and summing the dipolar moment contributions
of all charges, this static polarisation is output in the header of {\prg results/mcphas.fum}.

\begin{equation}
\mbf P_{\rm el0} = q_i  \langle \mbf r_i \rangle 
\end{equation}
...
\put(11.5,4.5) {$\mbf U_1$}
\put(26.3,8.5) {$\mbf U_2$}
...
Thus the phonon Hamiltonian (\ref{Hphonon}) can be separated into the elastic Energy $E_{\rm elastic}$, the
Einstein single ion oscillation term $H_{\rm E}$, the bilinear interaction term $H_{\rm int}$
and the mixing term $H_{mix}$ 

\begin{eqnarray}
\hat H_{\rm phon}& =&E_{\rm elastic} +\hat  H_{mix}+\hat  H_{\rm E}+\hat H_{\rm int}
\end{eqnarray}

\subsection{Elastic Energy $E_{\rm elastic}$}

The elastic energy $E_{\rm elastic}$ is bilinear in  $a$

\begin{eqnarray}\label{Eel}
E_{\rm elastic} &=& \frac{1}{2}\sum_{ij} \frac{c_L(ij)-c_T(ij)}{2|\mbf R_{ij}|^2} 
...
\begin{eqnarray}
\frac{\partial E_{\rm elastic}}{\partial a_{\alpha\beta}}&=& \frac{1}{2}\sum_{ij,\gamma\delta} \frac{c_L(ij)-c_T(ij)}{|\mbf R_{ij}|^2} 
R_{ij}^{\alpha}R_{ij}^{\beta}R_{ij}^{\gamma}R_{ij}^{\delta}
a_{\gamma\delta} \nonumber \\
&& +    \frac{c_T(ij)}{2} R_{ij}^{\alpha}R_{ij}^{\delta}
...
%\frac{\partial E_{\rm elastic}}{\partial a_{\alpha\beta}} + \frac{\partial E_{\rm elastic}}{\partial a_{\beta\alpha}}-\delta_{\alpha\beta}
%\frac{\partial E_{\rm elastic}}{\partial a_{\alpha\alpha}}  \\ 
...
An we make also use of the definition of elastic constants {\em (important note: these
are not the effective elastic constants, which can be determined experimentally, because
they do not take into account, that the atomic positions relax when the crystal is 
strained. To relax atomic positions and compute effective elastic constants use
option -cel of program {\prg mcphasit})}:

\begin{eqnarray}\label{elconst}
c^{\alpha\beta\gamma\delta} &= &
\frac{\partial^2E_{\rm elastic}}{\partial a_{\alpha\beta} \partial a_{\gamma\delta}} \\
...
E_{\rm elastic} &=& \frac{1}{2}\sum_{\alpha\beta\gamma\delta} c^{\alpha\beta\gamma\delta} \epsilon_{\alpha\beta}\epsilon_{\gamma\delta}
\end{eqnarray}
...
E_{\rm elastic} &=& \frac{1}{2}\sum_{\alpha\gamma=1,..,6} c^{\alpha\gamma} \epsilon_{\alpha}\epsilon_{\gamma}
\end{eqnarray}

\subsection{External Stress}

The stress tensor components  act similar as an external magnetic field in {\em McPhase}, 
these  only act on the
''espilon'' module and on no other module. It should be possible to calculate 
in addition to magnetic phase diagrams also stress dependence
of Jahn-Teller transitions and excitations, this has been programmed. 
If the system is subject to external stress $\sigma_{\alpha}$, the corresponding energy can be written as

\begin{equation}
E_{\rm stress}= -\sum_{\alpha=1,..,6} \sigma_{\alpha} \epsilon_{\alpha}
\end{equation}
...
Note that without crystal field phonon interaction and without external stress
 the mixing term $\hat H_{mix}$ does not contribute to the energy, because strains $\epsilon_{\alpha}$ and
expectation values $\langle \hat \mbf P_i \rangle$ are zero in this case. The last term reflects
...
defined writing explicitly the Voigt notation $\alpha \leftrightarrow (\alpha\beta)$ by
...
Similar to the elastic constants the Voigt notation is
 e.g. $G_{mix}^{(23)\gamma}(i)=G_{mix}^{4\gamma}(i)$.
...
&+&\sum_{i} \hat H_{\rm E}(i) + \frac{1}{2}\sum_{\alpha\gamma=1-6} c^{\alpha\gamma} \epsilon_{\alpha}\epsilon_{\gamma} -\sum_{\alpha=1,..,6} \sigma_{\alpha} \epsilon_{\alpha} -\nonumber \\
&-&\frac{1}{2}\sum_{i\ne j,\alpha\beta} K_{\alpha\beta}(ij) \hat u_{i}^{\alpha}\hat u_{j}^{\beta} 
-\sum_{i<j,\alpha=1,2,3,\gamma}\Gamma^{\alpha\gamma}(ij) \hat u^{\alpha}_i O_{\gamma}(\hat \mbf J_j)
\nonumber \\
...
\sum_{\beta=1,..6}  c^{\alpha\beta} \epsilon_{\beta} &=&\sigma_{\alpha}+
...
We therefore extend the model by the exchange striction introducing two ion interactions in the 
Hamiltonian, which depend on the strain tensor $\bar \epsilon$ and on the distance between
two ions:


\begin{equation}\label{Hwithexstric}
\hat H=\hat H_{\rm tot} -\frac{1}{2} \sum_{ij,\alpha\beta}
 {\mathcal J}_{\alpha\beta}(\bar \epsilon,(1+\bar a)(\mbf R_{j} - \mbf R_i)) \hat \mathcal I_{\alpha}^i \hat \mathcal I_{\beta}^{j}
\end{equation}
...
{\mathcal J}_{\alpha\beta}(\bar \epsilon,\mbf R=(1+\bar a)\mbf R_{ij})&=&{\mathcal J}_{\alpha\beta}(\bar \epsilon=0,\mbf R_{ij})
+ (\frac{\partial {\mathcal J}_{\alpha\beta}}{\partial \epsilon_{\beta'}}+\frac{\partial {\mathcal J}_{\alpha\beta}}{\partial R^{\alpha'}} \frac{\partial (\bar a\mbf R_{ij})^{\alpha'}}{\partial \epsilon_{\beta'}}) \epsilon_{\beta'} + \dots \nonumber \\
&=& {\mathcal J}_{\alpha\beta}(0,\mbf R_{ij})+ 
\sum_{\alpha',\beta'=1,..,6}
(\frac{\partial {\mathcal J}_{\alpha\beta}}{\partial \epsilon_{\beta'}}+
\sum_{\gamma=1,2,3}\frac{\partial {\mathcal J}_{\alpha\beta}}{\partial R^{\alpha'}}
...
\hat H=\hat H_{\rm tot} -\frac{1}{2} \sum_{ij,\alpha\beta}
 ...
 -\frac{1}{2} \sum_{ \stackrel{ij,\alpha\beta\alpha'}{\beta'=1,..,6}}
(\frac{\partial {\mathcal J}_{\alpha\beta}}{\partial \epsilon_{\beta'}}+
\sum_{\gamma=1,2,3}\frac{\partial {\mathcal J}_{\alpha\beta}}{\partial R^{\alpha'}}
\frac{\partial \epsilon_{\alpha'\gamma}\mbf R_{ij}^{\gamma}}{\partial \epsilon_{\beta'}}) \epsilon_{\beta'}
...
\sum_{\beta=1,..,6}  c^{\alpha\beta} \epsilon_{\beta} &=&\sigma_{\alpha}+
 ...
 &+& \sum_{i,\gamma=1,...} G_{\rm cfph}^{\alpha\gamma}(i) \langle O_{\gamma}(\hat \mbf J_i) \rangle \nonumber \\
 &+& \frac{1}{2}\sum_{ii',\delta,\delta'\alpha'}
(\frac{\partial {\mathcal J}_{\delta\delta'}(\mbf R_{ii'})}{\partial \epsilon_{\alpha}}+
\sum_{\gamma=1,2,3}\frac{\partial {\mathcal J}_{\delta\delta'}(\mbf R_{ii'})}{\partial R^{\alpha'}}
...
Tm, Ni and C by $q_{Tm}$, $q_{Ni}$ and $q_{C}$, respectively.
...
B_2^0 &=& {\rm Ni}_{20} q_{Ni}+{\rm C}_{20} q_{C}+{\rm Tm}_{20} q_{Tm} \nonumber \\
B_2^2 &=& {\rm Ni}_{22} q_{Ni}+{\rm C}_{22} q_{C}+{\rm Tm}_{22} q_{Tm} \nonumber \\
0 &=& q_{Ni}+2 q_{C}+ q_{Tm} 
\end{eqnarray}
...
The following code snippet shows how to calculate the coefficients
 ${\rm C}_{20}$ and ${\rm C}_{22}$ using {\prg cif2mcphas} in linux:
...
\includegraphics[bb=0 0 300 300,angle=0, width=0.4\textwidth]{../examples/tmnic2/figures/B20convergence.jpg}
\end{center}
...
\begin{figure}[htb]%h=here, t=top, b=bottom, p=separate figure page
\begin{center}\leavevmode
\includegraphics[bb=0 0 500 500, angle=0, width=0.4\textwidth]{../examples/tmnic2/figures/model.jpg}
\end{center}
...
B_2^0 &=& {\rm Ni}_{20} q_{Ni}+{\rm C}_{20} q_{C}+{\rm Tm}_{20} q_{Tm}+\sum_{i=1}^6 E^i_{20} q_{E^i} \nonumber \\
B_2^2 &=& {\rm Ni}_{22} q_{Ni}+{\rm C}_{22} q_{C}+{\rm Tm}_{22} q_{Tm} +\sum_{i=1}^6 E^i_{22} q_{E^i} \nonumber \\
0 &=& q_{Ni}+2 q_{C}+ q_{Tm} +q_{E^1}+2q_{E^2}+2q_{E^3}+4q_{E^4}+2q_{E^5}+2q_{E^6}
\end{eqnarray}


By variation of the charges $q_{E^i}$ a set of point charges and crystal field parameters
...
If {\prg mcphas} is used with option {\prg -doeps} and 
it finds files {\prg mcphas.djdx}, {\prg mcphas.djdy}, {\prg mcphas.djdz} with
...
equation (\ref{zeroepsex}) at each iteration. 

Similar, If {\prg mcphas} 
is used with option {\prg -doeps} and it finds files {\prg mcphas.djdeps1}, {\prg mcphas.djdeps2},\dots,
 {\prg mcphas.djdeps6} with
derivatives of the two ion interaction parameters with respect to
strain components in Voigt notation $\epsilon_1,\dots,\epsilon_6$ respectively,it performs a calculation of
the corresponding correlation functions and computes the strain  using
equation (\ref{zeroepsex}) at each iteration.

Subsequently, making use of the Taylor expansion
...
\begin{figure}[htb]%h=here, t=top, b=bottom, p=separate figure page
\begin{center}\leavevmode
\includegraphics[bb=0 0 300 300,angle=0, width=0.4\textwidth]{../examples/tmcu2_cf_phonon/figures/thecf.jpg}
\end{center}
...
\begin{figure}[htb]%h=here, t=top, b=bottom, p=separate figure page
\begin{center}\leavevmode
\includegraphics[bb=0 0 900 900,angle=0, width=0.4\textwidth]{../examples/tmcu2_cf_phonon/figures/thecf_tmcu2_exp.jpg}
\end{center}
\caption{Crystal field striction, i.e. the crystal field influence on the thermal expansion
...
\begin{figure}[htb]%h=here, t=top, b=bottom, p=separate figure page
\begin{center}\leavevmode
\includegraphics[bb=0 0 700 700,angle=0, width=0.6\textwidth]{../examples/tmcu2_cf_phonon/figures/mag20K.jpg}
\end{center}
...
\begin{figure}[htb]%h=here, t=top, b=bottom, p=separate figure page
\begin{center}\leavevmode
\includegraphics[bb=100 100 700 700,angle=0, width=0.6\textwidth]{../examples/tmcu2_cf_phonon/figures/TmCu2 20K.jpg}
\end{center}
