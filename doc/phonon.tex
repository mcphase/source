...
$\alpha \le \beta$. The other 60 elastic constants 
...
H & = & H_{\rm ph} +\sum_{j,\gamma} B_{\gamma}(j,\vec U_1,...,\vec U_N) O_{\gamma}(\vec J_i) \\
...
H & = & \sum_{i,\gamma} B_{\gamma}(i,0,\dots,0) O_{\gamma}(\vec J_i) + \sum_{i} H_{\rm E}(i) + \\
...
The first line in (\ref{H}) contains the single ion Hamiltonian (crystal field, phonon), 
...
\sum_{\gamma=1,..6}  c^{\alpha\gamma} \epsilon_{\gamma} &=&
 \sum_{i,\gamma=1,2,3}  G_{mix}^{\alpha\gamma}(i)\langle u_{i}^{\gamma} \rangle  \\
...
Technically the Hamiltonian (\ref{H}) is of the general form for 
