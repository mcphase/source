...
If ${\rm err}^2$ is given, the program minimizes {\em sta}$=\chi^2=1/N\sum_i \delta_i^2/{\rm err}^2$.
...
(i.e. 21.3 in our example) denotes a maximum value of the {\em sta}$\times N$, i.e. of either
 $\sum_i \delta_i^2$ or (in case ${\rm err}^2$ is given)
$\sum_i \delta_i^2/{\rm err}^2$ . The
user written program may use this number and stop, when in the summation process
to obtain standard deviation {\em sta}$\times N$ reaches a value larger than this maximum
...
the standard deviation($\times N$) 21.3 is reached, here an example code from {\prg calcsta}:
\begin{verbatim}
if ! mcphasit -doeps -cd  -stamax $1 -chi mcphas.htp
then
echo "sta=1000 1"
exit
fi
\end{verbatim}
Note that in this code snippet the variable \$1 contains the argument of the calcsta call by
{\prg simannfit} (i.e. the number corresponding to the maximum value of {\em sta}$\times N$, which when reached
should stop the execution of calcsta). In case this limit is reached during the executiong of {\prg mcphasit}
the program exits with error and the {\prg echo} statement will output a large $\delta_i^2=1000$ with
error ${\rm err}^2=1$ (i.e. this snippet is for a calcsta which returns two numbers after ''sta='').
\end{enumerate}
