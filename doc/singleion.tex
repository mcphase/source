...
\PP I always tell in my lectures that 
...
4f electrons of the rare earth and causes magnetic anisotropy. In addition
to the electrostatic origin of the crystal field there is also a contribution due to the 
bonding.

\EE Look, \pp, in Fig.~\ref{chrgpla} and
\ref{chrgplb} I can show the you the effect of the crystal field on the charge distribution.
Such plots can be made by using the programs {\prg pointc\index{pointc}} to calculate
the crystal field parameters and
 {\prg display\_density\index{display\_density}} to plot the charge density
\footnote{ see section \ref{addprog}}.
...
\OO \ee, I appreciate the effort to do such beautiful plots of the charge density, 
but with your kind permission may I ask what relevance this has for experiments ? 
Normally in the literature I find the following expressions for the crystal field Hamiltonian
\footnote{\dots within the $|JLSm_J \rangle$ ground state multiplet of the 4f electron wave function.
...
crystal field parameters $B_l^m$. }
...
\EE This is exactly what the {\prg so1ion}\index{so1ion} module  of {\prg McPhase}
 uses to evaluate quantitatively the crystal field influence.

 In the case of isolating materials the crystal field parameters $B_l^m$
 can be obtained by the point charge model\footnote{point charges on the 
...
the pointcharge model for a given crystal structure\footnote{see section~\ref{addprog})}.
...
\PP In my lectures I am using a simpler way of dealing with crystal field anisotropy,
it is to write instead of the first term in equ.~\ref{cfham}

\begin{equation}
  \sum_s D_x^2 (J_x^s)^2 + D_y^2 (J_y^s)^2 +D_z^2 (J_z^s)^2 
\end{equation}

\EE Honorable \pp, indeed this is also possible - yet it is currently not possible to
compute the parameters $D^2_{\alpha}$ from a point charge model.

\OO For a diagonalisation of the single ion crystal field Hamiltonian
the module we have always used the program {\prg so1ion}\index{so1ion}({\prg cfield\index{cfield}}, 
written by P. Fabi n\'e Hoffmann). It can calculate also transition elements
and  the inelastic neutron scattering cross section.

\EE Exactly this program
 has been included into {\prg McPhase}. It  can be used  to 
...
However, I would recommend how to use the program {\prg singleion} to do crystal field calculation
for rare earth ions, because setting up the calculation in this way it
 can be easily extended to intermediate coupling schemes if necessary.
...
# - variables and simple expressions can be used  such as for Dz2 shown below
#        supported math functions : sin, cos, tan, asin, acos, atan, sinh, 
#        cosh, tanh, asinh, acosh, atanh, ln, log, exp, sqrt, sqr, round, int.
#        (to check if input is correctly read look in output files starting 
#         with results/_  , e.g. results/_Nd3p.sipf )
...
% defined in define_j.c:    m =2J+1,...,1  => mj =J,...,-J
Here comes the output file {\prg results/Nd3p.sipf.levels.cef}, note that for the so1ion module
the row indices 1,....,d=10=2J+1 correspond to the quantum numbers -J,....,+J, respectively. This
means the first row corresponds in the example to the eigenvector components of $|J-9/2 MJ=-9/2\rangle$,
 the second row to $|J-9/2 MJ=-7/2\rangle$ and so on:
{\footnotesize
\begin{verbatim} 
#
#
...
Note: the susceptibility can also be calculated using the option {\prg -XM 0 0},
   the two zeroes stand for energy and imaginary part. Thus using this option,
   also the response to oscillating magnetic fields can be calculated,
   a useful option is {\prg -Esteps 20 30}, which calculates 20 energies up to
   30~meV. 
...
{\bf rhoso1ion -t 10:1:100 -r 0.2 -i results/Nd3p.sipf.levels.cef}
...
\PP I understand that the program {\prg singleion} can do crystal field calculations
in a standard way such as many other programs do - so what is so special about 
{\prg Mcphase} ?

\EE It is the possibility to couple several such calculations by two ion interactions and
describe various kinds of ordering phenomena and  dispersive excitations.

\OO In fact the experiments I do are on ferrimagnets, so the theory has to take 
account of magnetic order.

\EE  So we have to {\prg mcphas}: the use of the {\prg so1ion\index{so1ion}} module
 in {\prg mcphas} is necessary in order to 
...
\PP What do you actually mean by module ?

\EE A module in the sense of {\prg McPhase} is a program which is called by another program.
In particular,
...
\PP Ok, I understood what you mean by single ion module, but how can we do some calculation
in {\prg McPhase} using the crystal field parameters ?
...
Tools for rotating crystal field parameters are described in appendix~\ref{approtateBlm}.}
