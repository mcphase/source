...
\end{equation}
where 
%script2html_begin{singleion}
$\chi^{s}_{\alpha\beta}(\omega)$ is the
...
energy levels of the subsystem $s$ 
%script2html_end{singleion} .
as calculated 
...
and insert this into equation (\ref{singleionsusceptibility}). Technically this is done 
in the {\prg jjjpar::chi0()} function in source file {bin/src/jjjpar\_basmodfunc.cpp}.
...
Also program {\prg singleion} can be used
to evaluate the magnetic subsystem susceptibility according to (\ref{singleionsusceptibility}),
see section \ref{addprog}.
...
algorithm\cite{rotter12-213201,rotter06-400} to calculate excitation energies and intensities and store the result in {\prg mcdisp.qom, %%@
mcdisp.qei, etc.}. 
%script2html_end{singleion}
The flowing chart of such a calculation is shown in fig.~\ref{figdmdproc}
...
When using the program {\prg mcdisp} with option {\prg -X'observable'}, e.g. {\prg -Xpel}, 
the coefficients of the $\delta$-function in (\ref{Xabscalc}) are evaluated (note these
have the unit of the observable squared) and output into output file {\prg results/mcdisp.qeX'obserable'}.

\noindent  The correlation function \hili{$\M{\m{\Sigma}}$} can be evaluated by applying the 
