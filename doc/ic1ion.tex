...
# --- some faster algorithms for matrix diagonalisation to b e used by singleion and mcphasit
# mean field loops (there are to be done many diagonalisations and calculation of expectation values,
# this is done in the module functions Icalc and IMcalc). Note, the following fast matrix diagonalisation
# options have no effect on running mcdisp (module function estates).

# uncomment the next line to save matrices which are often used the first time they are
# calculated in the directory results/mms, and then reread for subsequent instances.
...
# Note: the truncate_matrix option in the sipf file will have no effect when using mcdisp
...
\item Also the inelastic neutron scattering cross section
 in {\prg mcdisp\index{mcdisp}} is evaluated using the first
...
Currently, the beyond dipole part of {\prg mcdisp} is not yet implemented, as are the calculations
